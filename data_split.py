...
def get_latest_data(data_str: str):
    """
    Accept the raw data_str from client
    Return the latest data patch in format [xxxx]
    """
    pattern = r'\[.*?\]'
    res = re.findall(pattern, data_str)
    print(res)
    if len(res) >= 1:
        return res[-1][1:-1]
    else:
        print(f"Too short information encountered!")
        return DEFAULT_ONE_HAND + "|" + DEFAULT_ONE_HAND 
