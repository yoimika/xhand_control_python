...
def camerapicture(rs_pipelines, dirs, frame):
    for ind, pipeline in enumerate(rs_pipelines):
        ...
        # 3. 保存到picture目录
        color_path = os.path.join(dirs, f"images/color_{ind}_{frame}.png")
        depth_path = os.path.join(dirs, f"images/depth_{ind}_{frame}.png")

        t1 = time.time()        
        #cv2.imwrite(color_path, color_img)
        im.imwrite(color_path, color_img, quality=25)
        print('bbbbbbbbbbbbb', (time.time() - t1) * 1000)
        print(f"保存路径: {color_path}")  # 调试输出
        '''
        depth_colormap = cv2.applyColorMap(
            cv2.convertScaleAbs(depth_img, alpha=0.03), 
            cv2.COLORMAP_JET
        )
        im.imwrite(depth_path, depth_colormap,quality=25)
        #cv2.imwrite(depth_path, depth_colormap)
        print(f"保存路径: {depth_path}")  # 调试输出'''


save_dir = "/home/robot/Desktop/picture"
print(f"尝试保存到: {save_dir}")
print(f"目录可写: {os.access(save_dir, os.W_OK)}")  # 检查权限
    ...
    while True:
        for ind, pipeline in enumerate(rs_pipelines):
            ...
            if frame_count % 100 == 0:
                color_path = os.path.join(save_dir, f"color_{ind}_{frame_count}.jpg")
                depth_path = os.path.join(save_dir, f"depth_{ind}_{frame_count}.jpg")
